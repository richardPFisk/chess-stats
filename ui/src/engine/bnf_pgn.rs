...
pub fn read_pgn(pgn_str: &str) {
let nope  = r#"<PGNDatabase> ::= <PGNGame> <PGNDatabase> | <empty>
    
    <PGNGame> ::= <tagSection> <movetextSection>


<movetextSection> ::= <elementSequence> <gameTermination>

<elementSequence> ::= <element> <elementSequence>
                      | <recursiveVariation> <elementSequence>
                      | <empty>

<element> ::= <moveNumberIndication>
              | <SANMove>
              | <numericAnnotationGlyph>

<recursiveVariation> ::= <recursiveStart> <elementSequence> <recursiveEnd>
...
"#;
    ...
    match grammar {
        Ok(g) => {
            println!("{:#?}", g);
            let parses: Vec<_> = g.parse_input(pgn_str).map(|a| a.to_string()).collect();
            println!("WHAT IS FUCKING UP? {parses:#?} \n{pgn_str}");
        },
        Err(e) => {
            println!("Failed to make grammar from String: {e:#?}")
        },
    }
}
