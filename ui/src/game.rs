...
pub struct ChessCompletedGameComponentProps {
    #[props(!optional)]
    game: Option<CompletedGame>,
}
...
fn create_chess_board_props(pgn: &str) -> Result<ChessGameComponentProps, Box<dyn Error>> {
  ...
  let (positions, final_pos) = san_moves.iter()
      .try_fold(
...
pub fn ChessGame(props: ChessGameComponentProps) -> Element {
  ...
  rsx! {
      div {
          ...
          ChessBoard { board: game_state.read().current_board().cloned() }
