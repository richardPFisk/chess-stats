use rschess::{errors::{InvalidFenError, InvalidPgnError}, Board, Fen, Pgn};
// use pleco::Board;
...
mod tests {
    ...
    fn test_board() {
      ...
      let r = read_fen(fen);
      println!("{r:#?}");
      assert_eq!(r.unwrap(), Board::default());
    }
    ...
    fn test_pgn() {
      ...
      let pgn= read_pgn(pgn);
      // let fen = pgn.unwrap().position().to_fen();
      println!("{pgn:#?}");
    }
}